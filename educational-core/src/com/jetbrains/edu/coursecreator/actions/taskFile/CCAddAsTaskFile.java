...
public class CCAddAsTaskFile extends CCTaskFileActionBase {
  ...
  private static class AddTaskFile extends TaskUndoableAction {
    ...
    public void performUndo() {
      ...
      CCHideFromStudent.hideFromStudent(file, myProject, task.getTaskFiles(), myTaskFile);
    ...
    public void performRedo() {
      if (myTaskFile != null) {
        task.addTaskFile(myTaskFile);
      } else {
        final String taskRelativePath = EduUtils.pathRelativeToTask(myProject, file);
        task.addTaskFile(taskRelativePath);
        myTaskFile = task.getTaskFile(taskRelativePath);
      }
      ProjectView.getInstance(myProject).refresh();
    }

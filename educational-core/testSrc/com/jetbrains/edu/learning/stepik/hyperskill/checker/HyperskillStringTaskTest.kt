...
class HyperskillStringTaskTest : CheckersTestBase<Unit>() {
    ...
    section("Topics") {
      lesson("1_lesson_correct") {
        stringTask(stepId = 1, name = "1_string_task") {
          taskFile(StringTask.ANSWER_FILE_NAME, text = "<p>answer</p>")
        }
        stringTask(stepId = 1, name = "2_string_task") {
          taskFile(StringTask.ANSWER_FILE_NAME, "<p></p>")
        }
      }
    }
  }
  ...
  fun `test creating placeholder`() {
    ...
    assertEquals(EduCoreBundle.message("string.task.comment.file"),
                 createdTask.getTaskFile(StringTask.ANSWER_FILE_NAME)?.answerPlaceholders?.first()?.placeholderText)
    assertEquals(0, createdTask.getTaskFile(StringTask.ANSWER_FILE_NAME)?.answerPlaceholders?.first()?.offset)
    assertEquals(EduCoreBundle.message("string.task.comment.file").length,
                 createdTask.getTaskFile(StringTask.ANSWER_FILE_NAME)?.answerPlaceholders?.first()?.endOffset)
  }

...
class GenerateTaskTestConnector {
  ...
  internal var testService: GenerateTaskTestService? = null

  internal val service: GenerateTaskTestService
    get() = testService ?: createGenerateTaskTestService()
  ...
  suspend fun generateTests(
    ...
    if (!response.isSuccessful) {
      ...
      when (response.code()) {
        ...
        HttpURLConnection.HTTP_UNAVAILABLE, HttpURLConnection.HTTP_BAD_GATEWAY -> error("Service is temporarily unavailable")

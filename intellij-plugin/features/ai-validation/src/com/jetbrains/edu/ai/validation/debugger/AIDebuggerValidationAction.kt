...
class AIDebuggerValidationAction : AnAction() {
  ...
  private fun List<DebuggerHintRequest>.writeCSV() {
    ...
    format.print(writer).use { printer ->
      printer.printRecord("authorSolution", "courseId", "programmingLanguage", "taskDescriptionText", "taskDescriptionFormat",
        "taskId", "testText", "testName", "testExpectedOutput", "testErrorMessage", "testFiles", "updateVersion", "userSolution")
      forEach {
        printer.printRecord(
          it.authorSolution,
          it.courseId,

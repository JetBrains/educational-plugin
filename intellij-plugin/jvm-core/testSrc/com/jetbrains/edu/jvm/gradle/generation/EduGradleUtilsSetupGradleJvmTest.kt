...
class EduGradleUtilsSetupGradleJvmTest : EduTestCase() {
  ...
  override fun setUp() {
    ...
    every { gradleProjectSettings.externalProjectPath } returns "/home/user/projects/project"
  ...
  fun `compatible auto detected or already configured sdk is used when project sdk is incompatible`() {
    ...
    val autoDetectedEntry = SdkType.SdkEntry(validJdkHome, "21")
    ...
    every { javaSdk.collectSdkEntries(any()) } returns listOf(autoDetectedEntry)
  ...
  private fun mockSdk(name: String, homePath: String, version: String): Sdk =
    ProjectJdkImpl(name, JavaSdk.getInstance(), homePath, version)

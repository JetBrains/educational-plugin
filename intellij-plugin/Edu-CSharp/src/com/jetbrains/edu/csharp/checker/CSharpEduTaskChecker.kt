...
class CSharpEduTaskChecker(task: EduTask, private val envChecker: EnvironmentChecker, project: Project) :
  ...
  suspend fun getRdUnitTestCriterion(): RdUnitTestCriterion? = withContext(Dispatchers.EDT) {
    val pathToTests = getTestPath(task.getDir(project.courseDir)) ?: return@withContext null

    val virtualFileUrl = WorkspaceModel.getInstance(project).getVirtualFileUrlManager()
      .findByUrl(pathToTests)
    if (virtualFileUrl == null) {
      LOG.error("No test directory found for task ${task.name}: $pathToTests")
      return@withContext null
    }
    val taskId = WorkspaceModel.getInstance(project).getProjectModelEntities(virtualFileUrl).firstOrNull()?.getId(project)

    if (taskId == null) {
      LOG.error("No project model entity associated with task ${task.name} found: $pathToTests")
      return@withContext null
    }

    return@withContext rdUnitTestCriterion(taskId)
  }

  private fun getTestPath(path: VirtualFile?): String? = if (project.isUnityProject.value) {
    path?.findChild(CSharpConfigurator.TEST_CS).toString()
  }
  else {
    path?.findChild(CSharpConfigurator.TEST_DIRECTORY).toString()
  }

  private val rdUnitTestCriterion: (Int) -> RdUnitTestCriterion
    get() = if (project.isUnityProject.value) {
      { id -> RdProjectFileCriterion(id) }
    }
    else
      { id -> RdProjectFolderCriterion(id) }

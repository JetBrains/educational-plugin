...
class CodeforcesStartupActivity : StartupActivity {
  ...
  override fun runActivity(project: Project) {
    if (project.isDisposed || !project.isStudentProject() || isUnitTestMode || project.course !is CodeforcesCourse) return

    val submissionsManager = SubmissionsManager.getInstance(project)

    if (submissionsManager.isLoggedIn()) {
      submissionsManager.prepareSubmissionsContentWhenLoggedIn { updateCheckStatus(project) }
    }

    project.messageBus.connect().subscribe(CodeforcesSettings.AUTHENTICATION_TOPIC, object : EduLogInListener {
      override fun userLoggedIn() = submissionsManager.prepareSubmissionsContentWhenLoggedIn { updateCheckStatus(project) }

      override fun userLoggedOut() = TaskToolWindowView.getInstance(project).updateTab(TabType.SUBMISSIONS_TAB)
    })

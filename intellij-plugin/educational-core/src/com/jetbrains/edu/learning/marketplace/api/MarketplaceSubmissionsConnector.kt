...
class MarketplaceSubmissionsConnector {
  ...
  private val submissionsServiceInternal: SubmissionsServiceInternal
    get() = submissionsServiceInternal()

  private fun submissionsServiceInternal(): SubmissionsServiceInternal {
    val retrofit = createRetrofitBuilder(submissionsServiceUrl, connectionPool, "internaltoken")
      .addConverterFactory(converterFactory)
      .build()

    return retrofit.create(SubmissionsServiceInternal::class.java)
  }

  suspend fun submitAgreementAcceptanceAnonymously(pluginAgreementAccepted: Boolean) {
    try {
      val response = submissionsServiceInternal.saveAgreementAcceptanceAnonymously(pluginAgreementAccepted)

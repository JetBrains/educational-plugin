...
fun calculateChanges(
  ...
  loop@ for ((path, nextText) in targetState) {
    ...
    changes += when {
      ...
      !currentText.textContentEquals(nextText) -> Change.ChangeFile(path, nextText)
      else -> continue@loop
    }
  }
...
fun FLTaskState.stateEquals(other: FLTaskState): Boolean {
  ...
  return all { (path, contents1) ->
    ...
    contents1.textContentEquals(contents2)
  }
}
 ...
 */
fun FileContents.textContentEquals(other: FileContents?): Boolean = textualRepresentation == other?.textualRepresentation

/**
 * Hash code for corresponding simple equality
 *
 * See: [FileContents.textContentEquals].
 */
val FileContents.textHashCode
  get() = textualRepresentation.hashCode()
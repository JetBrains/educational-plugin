...
interface EduConfigurator<Settings : EduProjectSettings> {
  ...
  fun archiveFileInfo(holder: CourseInfoHolder<out Course?>, file: VirtualFile): ArchiveFileInfo = buildArchiveFileInfo(holder, file) {
    ...
    when {
      ...
      filter { EduUtilsKt.isTaskDescriptionFile(it.name) } -> {
        description("Task description file")
        type(IncludeType.MUST_NOT_INCLUDE)
      }
      filter { isConfigFile(it) } -> {
...
fun EduConfigurator<*>.excludeFromArchive(holder: CourseInfoHolder<out Course?>, file: VirtualFile): Boolean {
  return archiveFileInfo(holder, file).excludedFromArchive == true
}
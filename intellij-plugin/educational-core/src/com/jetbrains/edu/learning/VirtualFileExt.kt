...
fun VirtualFile.getSection(holder: CourseInfoHolder<out Course?>): Section? {
  ...
  return if (holder.courseDir.findFileByRelativePath(course.contentShift) == parent) course.getSection(name) else null
}
...
fun VirtualFile.getLesson(holder: CourseInfoHolder<out Course?>): Lesson? {
  ...
  return if (holder.courseDir.findFileByRelativePath(course.contentShift) == parent) course.getLesson(name) else null
}

...
import com.jetbrains.edu.learning.marketplace.peekSolution.createCommunityLinksPanel
import com.jetbrains.edu.learning.marketplace.peekSolution.marketplacePeekSolutionLink
...
class CheckDetailsPanel(project: Project, task: Task, checkResult: CheckResult, alarm: Alarm) : JPanel(BorderLayout()) {
  ...
  private fun createAnswerHintsPanel(project: Project, task: Task, checkResult: CheckResult): JPanel? {
    ...
    val peekSolution = when {
      project.isMarketplaceStudentCourse() && task.canShowSolution() -> {
        marketplacePeekSolutionLink(checkResult.isSolved)
      }

      project.isStudentProject() && task.canShowSolution() -> {

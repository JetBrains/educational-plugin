...
class FrameworkLessonTaskHistory(project: Project, localLesson: FrameworkLesson, remoteLesson: FrameworkLesson) {
  ...
  init {
    ...
    val allTaskFiles = mutableSetOf<String>()
    localLesson.taskList.flatMapTo(allTaskFiles) { it.taskFiles.keys }
    remoteLesson.taskList.flatMapTo(allTaskFiles) { it.taskFiles.keys }
...
class FrameworkLessonTaskFileHistory(
  ...
  private val history: List<TaskFileStep?> = extractTaskFileHistory(remoteLesson, fileName)
  ...
  fun evaluateContents(
    ...
    if (isTemplateBased) {
      return userChanges[index] ?: history[index]?.authorContents
    }

    var currentIndex = index

    while (currentIndex >= 0) {
      val userChanges = userChanges[currentIndex]
      if (userChanges != null) return userChanges // user changes are the most important, so we always take them if they present

      val historyStep = history[currentIndex]
      if (historyStep == null) {
      ...
      if (currentIndex == 0 || history[currentIndex - 1]?.shouldBePropagated != true) return authorContents

...
abstract class CourseProjectGenerator<S : EduProjectSettings>(
  ...
  fun createAdditionalFiles(holder: CourseInfoHolder<Course>) {
    ...
    for (file in autoCreatedLaterAdditionalFiles(holder)) {
      val archiveInclusionPolicy = configurator.courseFileAttributes(file.pathInCourse, false).archiveInclusionPolicy
      if (archiveInclusionPolicy >= ArchiveInclusionPolicy.INCLUDED_BY_DEFAULT) {
        addAdditionalFile(file)
      }
    }
  ...
  /**
   * Returns the list of additional files that must be added to the project but will be created after the course creation stage.
   *
   * For example: `gradle/wrapper/gradleWrapper.properties` for CC is autogenerated by Gradle later.
   */
  open fun autoCreatedLaterAdditionalFiles(holder: CourseInfoHolder<Course>): List<EduFile> = emptyList()

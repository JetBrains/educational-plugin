...
data class CourseArchiveContent(
  ...
  fun assertEquals(
    ...
    val filesToCompare = mutableMapOf<String, Pair<ByteArray, ByteArray>>()
    ...
    for ((path, expectedContent) in expected.files) {
      ...
      if (actualContent == null) {
        ...
        filesToCompare[path] = expectedContent to actualContent
      }
    }
    ...
    for ((path, contents) in filesToCompare) {
      val (expectedContent, actualContent) = contents
      if (path == COURSE_META_FILE) {
        val expectedJson = expectedContent.toString(Charsets.UTF_8).withLatestJsonVersion()
        val actualJson = actualContent.toString(Charsets.UTF_8)
        ...
        Assert.assertArrayEquals("Unexpected $path content", expectedContent, actualContent)
      }
    }
  }

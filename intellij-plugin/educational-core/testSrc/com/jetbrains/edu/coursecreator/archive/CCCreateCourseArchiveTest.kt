...
class CCCreateCourseArchiveTest : CourseArchiveTestBase() {
  ...
  fun `test course archive creation with task file of wrong binarity`() {
    ...
    val action = notification.actions.single {
      ...
      it.templatePresentation.text == EduCoreBundle.message("action.make.file.binary")
    }
    ...
    assertTrue("cat.jpg file must become binary", course.allTasks[0].taskFiles["cat.jpg"]!!.isBinary!!)

  ...
  private fun doTest(
    ...
    fun VirtualFile.toEduFile(): EduFile? {
      ...
      return if (pathIsInsideTask) {
        ...
        val coursePath = pathInCourse(project)!!
    ...
    if (shouldActionBeEnabled) {
      for (visibleFile in affectedCourseFiles) {
        ...
        if (!action.requiredVisibility) {
          check((visibleFile as? TaskFile)?.answerPlaceholders?.isEmpty() != false) {
      ...
      for (visibleFile in affectedCourseFiles) {
        ...
        assertContentEquals(initialPlaceholders[visibleFile], (visibleFile as? TaskFile)?.answerPlaceholders)
      }
    }
  }
}

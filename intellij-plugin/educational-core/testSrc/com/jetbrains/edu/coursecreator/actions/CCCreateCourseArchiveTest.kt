...
class CCCreateCourseArchiveTest : CourseArchiveTestBase() {
  ...
  @Test
  fun `user created file is added to additional files in the educator mode`() {
    val course = courseWithFiles(courseMode = CourseMode.EDUCATOR) {}
    createUserFile("file.txt")
    assertEquals("file.txt", course.additionalFiles[0].name)
  }

  @Test
  fun `user created file is not added to additional files in the student mode`() {
    val course = courseWithFiles(courseMode = CourseMode.STUDENT) {}
    createUserFile("file.txt")
    assertEmpty(course.additionalFiles)
  }

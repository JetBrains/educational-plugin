...
class MarketplaceSubmissionsTest : SubmissionsTestBase() {
  ...
  fun `test DeleteSubmissionsAction - Deleting Course Submissions`() {
    configureSubmissionsResponses(submissionsLists = listOf(), submissionsDeleteRequestSuccess = true)
    createEduCourse()
    val submissionsManager = SubmissionsManager.getInstance(project)
    assertNullOrEmpty(submissionsManager.getSubmissionsFromMemory(setOf(1, 2)))

    checkTask(0, 0)
    checkTask(0, 1)
    TimeoutUtil.sleep(1000)

    checkSubmissionsPresent(submissionsManager, 1)
    checkSubmissionsPresent(submissionsManager, 2)

...
class LtiCourseMetadataProcessorTest : CourseMetadataProcessorTestBase() {
  ...
  fun `lti metadata`() {
    ...
    assertEquals(-1, StudyItemSelectionService.getInstance(project).lastSetCurrentStudyItem())
  }
  ...
  fun `lti metadata with study item id`() {
    ...
    assertEquals(12345, StudyItemSelectionService.getInstance(project).lastSetCurrentStudyItem())
  }
  ...
  fun `missing required parameters`() {
    ...
    assertNull(StudyItemSelectionService.getInstance(project).lastSetCurrentStudyItem())
  }

...
abstract class FrameworkLessonsUpdateTest<T : Course> : UpdateTestBase<T>() {
  ...
  fun `update unmodified current task if it is second`() {
    ...
    assertTaskFolder {
      ...
      file("NonEdit.kt", "val p = 41") // the file became propagatable after the update, so we propagate from the previous task (??)
      file("Tests2.kt", "fun test2() {}")
    }
  }
  ...
  fun `update modified current task`() {
    ...
    assertTaskFolder {
      file("Task.kt", "/*comment*/fun task() {}")
      file("NonEdit.kt", "val p = 41") // the file became propagatable after the update, so we see the old contents
      file("Tests1.kt", "fun test1() {}")
    }
  }
